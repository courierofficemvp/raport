    ...
    for sheet in ["CITI", "Toyoty", "skutery"]:
        ...
        for row in rows[1:]:
            if len(row) > 5:
                ...
                # pomijamy zakończonych
                if row[5].strip().lower() == "zakończony":
                    continue
...
def find_flota(plate):
    ...
    for row in rows[1:]:
        ...
        if len(row) > 0 and row[0].strip().lower() == plate:

            vin = row[1] if len(row) > 1 else ""
            company = row[2] if len(row) > 2 else ""
            model = row[3] if len(row) > 3 else ""
            marka = row[4] if len(row) > 4 else ""

            nr_polisy = row[5] if len(row) > 5 else ""
            nazwa_polisy = row[6] if len(row) > 6 else ""
            data_polisy = row[7] if len(row) > 7 else ""
            ...
            # polisa
            if nr_polisy or nazwa_polisy or data_polisy:

                text += "\n"
                text += line + "\n🛡️ POLISA\n" + line + "\n"

                if nr_polisy:
                    text += "Nr polisy: " + nr_polisy + "\n"

                if nazwa_polisy:
                    text += "Nazwa polisy: " + nazwa_polisy + "\n"

                if data_polisy:
                    text += "Data polisy: " + data_polisy + "\n"
...
async def handle(m: Message):
    ...
    if uid in flota_users:
        ...
        await m.answer(
            res if res else "❌ Nie znaleziono"
        )
